...
function Avatar({ p, size = 32 }: { p: Profile; size?: number }) {
  return (
    ...
    >
      {p.avatarUrl ? <img src={p.avatarUrl} alt="" className="h-full w-full object-cover" /> : <Initials p={p} />}
    </span>
  );
}

export function EditorProfileMenu({ initial }: { initial: Profile }) {
  const router = useRouter();
  const navigate = useNavigate();
  const fetchSession = useServerFn(getEditorSessionFn);
  const saveProfile = useServerFn(updateEditorProfileFn);
  const logout = useServerFn(logoutEditorFn);
      ...
      <div ref={wrapperRef} className="relative">
        ...
        >
          ...
          <span className="flex-1 min-w-0 text-left">
            <span className="block text-xs font-medium text-white truncate">{profile.displayName}</span>
  ...
  const modal = (
    ...
    >
      <motion.form
        ...
        className="w-[92vw] max-w-3xl rounded-2xl border border-white/10 bg-neutral-950 p-8 shadow-2xl"
        style={{ zoom: 1.25 }}
      >
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-white text-xl font-semibold">Mon profil</h2>
          <button type="button" onClick={onClose} disabled={saving} className="text-neutral-500 hover:text-white transition">
            <X className="h-5 w-5" />
          </button>
        </div>
